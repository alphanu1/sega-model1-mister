...
| `04`–`07` | Addresses of instruction fetches 2 to 5 | Captured once at boot and held. A CPU that dies does so in the first handful of fetches, and by the time anyone can photograph the screen the evidence is gone. Healthy: `0BFFFC`, `000000`, `0B0824`, `0B0828`. Rows `02` and `03` held fetches 0 and 1; they found the SDRAM off-by-one and then showed the same two constants for months, so the space went to the two questions above. |
