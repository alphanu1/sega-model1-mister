...
| `02` | Coprocessor microcode: words loaded (left 12 bits), folded checksum (right 12) | **`800`** is a complete 2,048-word load and the checksum then identifies *which* 8 KB — `B9A` is `315-5573.bin`. **`000` means the HPS never sent index 1** and the coprocessor is running an empty RAM, which fills its command FIFO and halts the V60. Nothing else can see this: MiSTer does not log ROM assembly and `/media/fat` is `noatime`. |
| `03` | Coprocessor command FIFO: pushes (left) against **drains** (right) | Drains counts the TGP **taking** a command — `fifo_in_pop`. It is deliberately NOT `dbg_fifo_pops`, which counts the V60 reading results back out. Reading that zero as "the TGP never drains its FIFO" produced a confident wrong diagnosis; check what a counter increments on, not what its name suggests. **AND ITS ZERO IS NOT CORRECT EITHER.** This row used to say the V60 "never reads the coprocessor back — 0 in 2,500 accesses, measured". That census was of the program space; the game uses `in.w`, so the traffic is in the V60's **I/O space**, where the reference reads `0xd80000` 710,722 times per 600 frames. A zero here is a real gap, not a property of the hardware — corrected 2026-08-18, `tools/mame_v60_iospace.lua`. |
| `04` | PC when the screen last tore down | `000000` means it never has. |
| `05` | How many times it has torn down | `000000` is healthy. |
| `06` | Cycles with the PC in boot ROM | Rising is normal — the reference's PC is in ROM0 essentially all the time too. Read it **against `07`**. |
| `07` | Cycles with the PC anywhere else | `000000` with `06` climbing means the CPU never leaves ROM0. On 2026-08-18 this pair independently confirmed the coprocessor deadlock, from a counter that knows nothing about the coprocessor. **These four rows were boot fetch addresses until they were repurposed, and this table described the old meaning for some time afterwards.** |
| `08` | Reset vector data, low 24 bits of `fd[1]` | **`4EF3D6`** was recorded as the healthy value, with `FE104E` the same burst read one 16-bit word late — see below. But a board booting correctly on 2026-08-18 read **`0D000D`**, which is neither, while everything downstream said memory was fine: it boots, fetches, loads microcode with the right checksum and draws. So either the recorded value is stale or this row no longer holds what it says. **Do not read it as a fault without first checking what `fd[1]` now captures.** |
| `0B` | Flags and I/O replies | Top byte after the tag: bit 5 loader overflow, bit 4 CPU halted, bit 3 FP trap, bit 2 ROM ready, bit 1 memory ready, bit 0 downloading. So `06` is the healthy value — ROM and memory ready, not halted, no trap. Low four digits are the I/O board's reply count. **`0001` is the healthy value and it should NOT climb** — measured: the reference's board clears the flag once, 38.6 ms into boot, and never again, because the V60's once-a-frame doorbell re-arms the deadline faster than it expires. A climbing count now means `LATENCY` is below the doorbell period; `0000` means the boot handshake never completed. This row read a climbing count for months against a 64-cycle guess. |
...
| The TGP counters WRAP where the census counters SATURATE | `dbg_retires`, `dbg_fifo_pops` and `dbg_fifo_returns` are `+ 16'd1` with no ceiling, so they roll over silently. A saturated `FFFF` at least looks pinned; a wrapped value looks like a **plausible reading** — `retires=57660` was `57,660 + 65,536k` and cannot be divided by a cycle count. Measured 2026-08-29: the same run reported 57,660 through the port and 385,340 on a 64-bit counter in the bench. Use these rows to tell MOVING from STOPPED, never as a quantity. |
