...
| `03` | Coprocessor command FIFO: pushes (left) against **drains** (right) | Drains counts the TGP TAKING a command — `fifo_in_pop`. It is NOT `dbg_fifo_pops`, which counts the V60 reading results back out and is correctly zero, because the V60 never reads the coprocessor back (0 in 2,500 accesses, measured). Reading that zero as "the TGP never drains" produced a whole wrong diagnosis. |
