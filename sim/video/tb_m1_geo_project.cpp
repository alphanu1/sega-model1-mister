...
int main(int argc, char** argv) {
    ...
    printf("test: THROUGHPUT streamed - the reciprocal stage must not idle\n");
    {
        ...
        // mistake this project already made once, in tb_m1_geo_xform.
        const int N = 400;
        ...
        printf("  peak frame needs %.0f cycles of the 397,515 available (%.0f%%)\n",
               per * 2 * 5831, 100.0 * per * 2 * 5831 / 397515.0);
        checks++;
        if (per * 2 * 5831 > 397515.0) {
            fails++;
            printf("  FAIL OVER BUDGET by %.2fx\n", per * 2 * 5831 / 397515.0);
        }
    }

    printf("m1_geo_project: checks=%ld fails=%ld\n", checks, fails);
    return fails ? 1 : 0;
}
