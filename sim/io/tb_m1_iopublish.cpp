...
int main(int argc, char** argv) {
  ...
  printf("test: the identity block is pushed to 0x100-0x17f\n");
  {
    ...
    //
    // THAT SAFEGUARD DID NOT WORK, because typing it out twice from the SAME
    // READING is still one reading. The first transcription caught the window
    // mid-push while the Z80 was still filling it, so six bytes were wrong here,
    // in the RTL and in the docs together — including 0x0b, which the V60 copies
    // to 0x40DC8B and tests at FF9737 to decide whether to use the coprocessor at
    // all. This table is re-measured (tools/mame_idblock.lua) and confirmed
    // STABLE at frames 10, 120 and 900, which is the check the first one lacked.
    //
    // Written out to all 0x80 entries rather than trailing off after 0x120: the
    // old table stopped there and so could not express 0x7c = 0x02 even if the
    // reading had been right.
    static const uint8_t BLOCK[0x80] = {
      0x53, 0x45, 0x47, 0x41, 0x1c, 0x82, 0x01, 0x00,   // 0x00
      0x88, 0x9a, 0xff, 0x01, 0x00, 0x00, 0x00, 0x00,   // 0x08
      0x00, 0x01, 0x01, 0x01, 0x00, 0x01, 0x01, 0xff,   // 0x10
      0xff, 0xff, 0x01, 0x02, 0x00, 0x00, 0x00, 0x00,   // 0x18
      ...
      0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00,   // 0x78 — 0x7c = 0x02
    };
    Dut t;
    t.run(PASS);
